 ...
 *
 * 演示内容：
 *   - EngineEditor 自动布局（SceneHierarchy / Inspector / Viewport / Console
 * 等）
 *   - 主菜单栏 + 工具栏
 *   - 面板可见性切换
 */
...
protected:
  ...
  void OnUpdate(float32 dt) override { (void)dt; }

  void OnRender() override {
    m_Shader.reset(); // ← 清空 shader，InternalRender 中的 if 条件不成立
    m_VAO.reset();    //     从而跳过默认四边形绘制
    m_Texture.reset();
